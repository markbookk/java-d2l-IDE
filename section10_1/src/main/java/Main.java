...
public class Main {
    ...
    public static Figure showHeatmaps(NDArray matrices, String xLabel, String yLabel, String[] titles, int width, int height) {
        ...
        for (int i = 0; i < numRows; i++) {
            for (int j = 0; j < numCols; j++) {
                ...
                double[][] matrix = new double[(int) NDMatrix.getShape().get(0)][(int) NDMatrix.getShape().get(1)];
        ...
        Grid grid = Grid.builder().columns(numCols).rows(numRows).pattern(Grid.Pattern.INDEPENDENT).build();
